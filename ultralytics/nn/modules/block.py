...
from mmdet.models.backbones.swin import ShiftWindowMSA, SwinBlockSequence
...
class C2PSA_WinAttn(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, e: float = 0.5):
        ...
        self.m = nn.Sequential(
            *(
                PSABlock_WinAttn(self.c, attn_ratio=0.5, num_heads=self.c // 64, shift=False if i % 2 == 0 else True)
                for i in range(n)
            )
        )
...
class ChannelConcat(nn.Module):
    ...
    def forward(self, x):
        ...
        for b in range(B):
            ...
            selected = [feats[i][b : b + 1] for i in idx]
