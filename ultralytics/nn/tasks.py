...
class CropBaseModel(torch.nn.Module):
    """Base class for all YOLO models in the Ultralytics family.
    ...
    def loss(self, batch, preds=None):
        """batch: { "samples": [ { "crop_samples": [crop1, crop2, ...] }, ... ] }.
        """
        if getattr(self, "criterion", None) is None:
            self.criterion = self.init_criterion()
        ...
        for sample in batch["samples"]:
            crop_samples = sample["crop_samples"]
            if len(crop_samples) == 0:
                continue

            sample_loss = None
            sample_loss_items = None

            for crop_batch in crop_samples:
                crop_img = crop_batch["img"]
                if crop_img.ndim == 3:
                    crop_img = crop_img.unsqueeze(0)  # [1, C, H, W]
                crop_batch["img"] = crop_img

                preds_i = self.forward(crop_img)
                loss_i, loss_items_i = self.criterion(preds_i, crop_batch)

                if sample_loss is None:
                    sample_loss = loss_i
                    sample_loss_items = loss_items_i
                else:
                    sample_loss = sample_loss + loss_i
                    sample_loss_items = sample_loss_items + loss_items_i

            # ===== 这里控制你要“叠加”还是“平均” =====
            # 1) 如果你要12个子图loss直接叠加，就不要除
            # 2) 如果你要每张原图内部对12个子图loss取平均，就除以 len(crop_samples)

            sample_loss = sample_loss / len(crop_samples)
            sample_loss_items = sample_loss_items / len(crop_samples)

            if total_loss is None:
                total_loss = sample_loss
                total_loss_items = sample_loss_items
            else:
                total_loss = total_loss + sample_loss
                total_loss_items = total_loss_items + sample_loss_items

        total_loss = total_loss / num_samples
        total_loss_items = total_loss_items / num_samples
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=6, nc=None, verbose=True):
...
class CropDetectionModel(CropBaseModel):
    """YOLO detection model.
...
class MidFusionDetectionModel(BaseModel):
    """YOLO detection model.
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=6, nc=None, verbose=True):
        ...
        self.branch_1 = 0
        self.branch_2 = 9
        self.fusion_start = 18
        # self.branch_1 = 0
        # self.branch_2 = 10
        # self.fusion_start = 20
        self.yaml = cfg if isinstance(cfg, dict) else yaml_model_load(cfg)  # cfg dict
        if self.yaml["backbone"][0][2] == "Silence":
            LOGGER.warning(
                "YOLOv9 `Silence` module is deprecated in favor of torch.nn.Identity. "
        ...
        if nc and nc != self.yaml["nc"]:
            LOGGER.info(f"Overriding model.yaml nc={self.yaml['nc']} with nc={nc}")
            self.yaml["nc"] = nc  # override YAML value
        self.model, self.save = parse_model_midfusion(
            deepcopy(self.yaml), ch=ch, branch=[self.branch_1, self.branch_2], verbose=verbose
        )  # model, savelist
        self.names = {i: f"{i}" for i in range(self.yaml["nc"])}  # default names dict
        self.inplace = self.yaml.get("inplace", True)
        self.end2end = getattr(self.model[-1], "end2end", False)
    ...
    def predict(self, x, profile=False, visualize=False, augment=False, embed=None):
        x0 = x[:, :3]
        x1 = x[:, 3:]
        return self._predict_once_midfusion(x0, x1, profile, visualize, embed)

    def _predict_once_midfusion(self, x0, x1, profile=False, visualize=False, embed=None):
        y, dt, embeddings = [], [], []
        embed = frozenset(embed) if embed is not None else {-1}
        max_idx = max(embed)
        for m in self.model:
            if m.f != -1:
                ...
                if m.i == self.branch_1:
                    x = x0
                elif m.i == self.branch_2:
                    x = x1
            if profile:
                self._profile_one_layer(m, x, dt)
            ...
            if m.i in embed:
                embeddings.append(torch.nn.functional.adaptive_avg_pool2d(x, 1).squeeze(-1).squeeze(-1))
                if m.i == max_idx:
                    return torch.unbind(torch.cat(embeddings, 1), dim=0)
        return x
...
class MidFusionMoEDetectionModel(BaseModel):
    """YOLO detection model.
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=6, nc=None, verbose=True):
        ...
        self.branch.append(9)
        for i in range(2, len(self.ch)):
            self.branch.append(self.branch[i - 1] + 3)
        self.fusion_start = 33
        self.yaml = cfg if isinstance(cfg, dict) else yaml_model_load(cfg)  # cfg dict
        if self.yaml["backbone"][0][2] == "Silence":
            LOGGER.warning(
                "YOLOv9 `Silence` module is deprecated in favor of torch.nn.Identity. "
        ...
        if nc and nc != self.yaml["nc"]:
            LOGGER.info(f"Overriding model.yaml nc={self.yaml['nc']} with nc={nc}")
            self.yaml["nc"] = nc  # override YAML value
        self.model, self.save = parse_model_midfusion_moe(
            deepcopy(self.yaml), ch=self.ch, branch=self.branch, verbose=verbose
        )  # model, savelist
        self.names = {i: f"{i}" for i in range(self.yaml["nc"])}  # default names dict
        self.inplace = self.yaml.get("inplace", True)
        self.end2end = getattr(self.model[-1], "end2end", False)
    ...
    def predict(self, x, profile=False, visualize=False, augment=False, embed=None):
        x0 = x.clone()
        return self._predict_once_midfusion(x, x0, profile, visualize, embed)

    def _predict_once_midfusion(self, x, x0, profile=False, visualize=False, embed=None):
        y, dt, embeddings = [], [], []
        embed = frozenset(embed) if embed is not None else {-1}
        max_idx = max(embed)
        for m in self.model:
            if m.f != -1:
                ...
                if m.i == self.branch[0]:
                    x = x0[:, :3]
                elif m.i != self.branch[0] and m.i in self.branch:
                    index = self.branch.index(m.i)
                    x = x0[:, sum(self.ch[:index]) : sum(self.ch[: index + 1])]
            if profile:
                self._profile_one_layer(m, x, dt)
            x = m(x)
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            args = [ch[f]]
        elif m in frozenset({Concat, ChannelConcat}):
            c2 = sum(ch[x] for x in f)
        elif m is ADD:
            c2 = max(ch[x] for x in f)
        elif m in frozenset({FusionADD, FusionADD_TokenLevel}):
            c2 = max(ch[x] for x in f[1:])
        elif m in frozenset(
...
def parse_model_midfusion(d, ch, branch, verbose=True):
    """Parse a YOLO model.yaml dictionary into a PyTorch model.
...
def parse_model_midfusion_moe(d, ch, branch, verbose=True):
    """Parse a YOLO model.yaml dictionary into a PyTorch model.
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if i in branch:
                c1 = ch_list[branch.index(i)]  # branch 1 input
            else:
                c1 = ch[f]
            ...
            c1 = ch[f]
            args = [*args[1:]]
        elif m in frozenset({VMoERouter, VMoETopKRouter}):
            c2 = args[0]
            c1 = ch[f]
            args = [c1, *args]
        else:
