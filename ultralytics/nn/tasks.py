...
class MidFusionDetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=6, nc=None, verbose=True):
        ...
        if nc and nc != self.yaml["nc"]:
            LOGGER.info(f"Overriding model.yaml nc={self.yaml['nc']} with nc={nc}")
            self.yaml["nc"] = nc  # override YAML value
        self.model, self.save = parse_model_midfusion(deepcopy(self.yaml), ch=ch, branch=[self.branch_1, self.branch_2], verbose=verbose)  # model, savelist
        self.names = {i: f"{i}" for i in range(self.yaml["nc"])}  # default names dict
        self.inplace = self.yaml.get("inplace", True)
        self.end2end = getattr(self.model[-1], "end2end", False)
...
class MidFusionMoEDetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=6, nc=None, verbose=True):
        ...
        if nc and nc != self.yaml["nc"]:
            LOGGER.info(f"Overriding model.yaml nc={self.yaml['nc']} with nc={nc}")
            self.yaml["nc"] = nc  # override YAML value
        self.model, self.save = parse_model_midfusion_moe(deepcopy(self.yaml), ch=self.ch, branch=self.branch, verbose=verbose)  # model, savelist
        self.names = {i: f"{i}" for i in range(self.yaml["nc"])}  # default names dict
        self.inplace = self.yaml.get("inplace", True)
        self.end2end = getattr(self.model[-1], "end2end", False)
