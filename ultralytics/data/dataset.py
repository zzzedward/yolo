...
from .base import BaseDataset, CropBaseDataset, MidfusionBaseDataset, MultiChannelBaseDataset
...
class YOLOMultiChannelDataset(MultiChannelBaseDataset):
    """Dataset class for loading object detection and/or segmentation labels in YOLO format.
    ...
    def cache_labels(self, path: Path = Path("./labels.cache")) -> dict:
        ...
        Returns:
            (dict): Dictionary containing cached labels and related information.
        """
        x = [[] for _ in range(len(path))]
        for i in range(len(path)):
            x[i] = {"labels": []}
            nm, nf, ne, nc, msgs = 0, 0, 0, 0, []  # number missing, found, empty, corrupt, messages
            desc = f"{self.prefix}Scanning {path[i].parent / path[i].stem}..."
            total = len(self.im_files[i])
            nkpt, ndim = self.data.get("kpt_shape", (0, 0))
            if self.use_keypoints and (nkpt <= 0 or ndim not in {2, 3}):
                raise ValueError(
                    "'kpt_shape' in data.yaml missing or incorrect. Should be a list with [number of "
            ...
            if msgs:
                LOGGER.info("\n".join(msgs))
            if nf == 0:
                LOGGER.warning(f"{self.prefix}No labels found in {path[i]}. {HELP_URL}")
            x[i]["hash"] = get_hash(self.label_files[i] + self.im_files[i])
            x[i]["results"] = nf, nm, ne, nc, len(self.im_files[i])
            x[i]["msgs"] = msgs  # warnings
            save_dataset_cache_file(self.prefix, path[i], x[i], DATASET_CACHE_VERSION)
        return x
    ...
    def get_labels(self) -> list[dict]:
        ...
        """
        self.label_files = img2label_paths(self.im_files)
        if isinstance(self.label_files[0], list):
            cache_path = [[] for _ in range(len(self.label_files))]
            for i in range(len(cache_path)):
                cache_path[i] = Path(self.label_files[i][0]).parent.with_suffix(".cache")
        else:
            cache_path = Path(self.label_files[0]).parent.with_suffix(".cache")
        try:
            cache, exists = load_dataset_cache_file(cache_path), True  # attempt to load a *.cache file
            assert cache[0]["version"] == cache[1]["version"] == DATASET_CACHE_VERSION  # matches current version
            assert cache[0]["hash"] == cache[1]["hash"] == get_hash(self.label_files + self.im_files)  # identical hash
        except (FileNotFoundError, AssertionError, AttributeError):
            cache, exists = self.cache_labels(cache_path), False  # run cache ops
        # Display cache
        for i in range(len(cache)):
            nf, nm, ne, nc, n = cache[i].pop("results")  # found, missing, empty, corrupt, total
            if exists and LOCAL_RANK in {-1, 0}:
                d = f"Scanning {cache_path}... {nf} images, {nm + ne} backgrounds, {nc} corrupt"
                TQDM(None, desc=self.prefix + d, total=n, initial=n)  # display results
                if cache[i]["msgs"]:
                    LOGGER.info("\n".join(cache[i]["msgs"]))  # display warnings

            # Read cache
            [cache[i].pop(k) for k in ("hash", "version", "msgs")]  # remove items
            labels = cache[i]["labels"]
            if not labels:
                raise RuntimeError(
                    f"No valid images found in {cache_path}. Images with incorrectly formatted labels are ignored. {HELP_URL}"
                )
...
class YOLOCropDataset(CropBaseDataset):
    """Dataset class for loading object detection and/or segmentation labels in YOLO format.
    ...
    def get_labels(self) -> list[dict]:
        """返回原图级别的 labels。 不在这里展开 crop，crop 放到 get_image_and_label() 里动态生成。.
        """
        self.label_files = img2label_paths(self.im_files)
        cache_path = Path(self.label_files[0]).parent.with_suffix(".cache")
        ...
        self.orig_im_files = self.im_files[:]  # 保留原图列表
        ...
        LOGGER.info(f"{self.prefix}Original images: {len(labels)}, crop_size={self.crop_size}, overlap={self.overlap}")
        return labels
    ...
    @staticmethod
    def collate_fn(batch: list[dict]) -> dict:
        """Batch 中每个元素对应一张原图，内部含多个 crop_samples。 这里保留原图级结构，后续在 BaseModel.loss() 中逐原图、逐 crop 计算 loss。.
        """
        return {"samples": batch}


class YOLOMidfusionDataset(MidfusionBaseDataset):
    """Dataset class for loading object detection and/or segmentation labels in YOLO format.
            ...
            if k in {"img", "text_feats"}:
                if k == "img":
                    if isinstance(value[0], (list, tuple)):
                        # value: [ [img0, img1], [img0, img1], ... ]
                        n_branch = len(value[0])
                        value = [torch.stack([v[i] for v in value], 0) for i in range(n_branch)]
                    else:
                        value = torch.stack(value, 0)
                elif k == "text_feats":
                    value = torch.stack(value, 0)
